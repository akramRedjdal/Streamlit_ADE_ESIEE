...
MODALITY_ORDER = ['CM', 'CM/TD', 'TD', 'TDR', 'TP', 'TP Seul', 'Oraux', 'Soutenance', 'Autre']

# Coefficients HETD : 1h de cours × coeff = HETD
# TD=1 (référence), CM=1.5, TP=1/1.5≈0.667, TP Seul=1, TDR=1.5, Oraux=1/1.5, Soutenance=1/1.5
# CM/TD et CM-plus assimilés CM (1.5), Autre = non comptabilisé (0)
HETD_COEFFICIENTS = {
    'CM':         1.5,
    'CM/TD':      0,
    'CM-plus':    1.5,
...
def write_detail_sheet(ws, records):
    headers = ['Nom', 'Date', 'Debut', 'Fin', 'Duree (h)', 'HETD (h)', 'Lieu', 'Modalite', 'Description']
    ...
    col_widths = [42, 12, 8, 8, 10, 10, 20, 10, 60]
...
def write_recap_sheet(ws, records):
    ...
    headers = ['Modalite', 'Coeff HETD', 'Nombre de seances', 'Heures totales', 'HETD totales']
    for col, h in enumerate(headers, 1):
        _header_cell(ws.cell(row=1, column=col), h)
    ...
    for m in present:
        ...
        vals  = [m, coeff, counts[m], round(hours[m], 2), round(hetd_hours[m], 2)]
    ...
    # TOTAL row
    for col, val in enumerate(['TOTAL', '', total_count, round(total_hours, 2), round(total_hetd, 2)], 1):
        cell = ws.cell(row=row, column=col, value=val)
        cell.font      = Font(bold=True)
...
def print_summary(records):
    ...
    print()
    print(sep)
    print(f"| {'Modalite':<{w1-2}} | {'Nombre de seances':^{w2-2}} | {'Heures totales':^{w3-2}} | {'HETD':^{w4-2}} |")
    print(sep)
    for m in present:
        print(f"| {m:<{w1-2}} | {counts[m]:^{w2-2}} | {hours[m]:^{w3-3}.2f} | {hetd_h[m]:^{w4-3}.2f} |")
    print(sep)
    print(f"| {'TOTAL':<{w1-2}} | {sum(counts.values()):^{w2-2}} | {sum(hours.values()):^{w3-3}.2f} | {sum(hetd_h.values()):^{w4-3}.2f} |")
    print(sep)
